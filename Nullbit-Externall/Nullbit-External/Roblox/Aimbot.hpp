...
void MakeAimbot() {
    ...
    std::string PlrName = globals::PlrName;
    ...
    // If movement is too small, no need to move mouse
    ...
    // Apply easing factor that scales more for bigger deltas to reduce snapping
    float easedDx = dx / (aimSpeed * (1.0f + fabsf(dx) * 0.1f));
    float easedDy = dy / (aimSpeed * (1.0f + fabsf(dy) * 0.1f));

    INPUT input = { 0 };
    input.type = INPUT_MOUSE;
    input.mi.dwFlags = MOUSEEVENTF_MOVE;
    input.mi.dx = static_cast<LONG>(roundf(easedDx));
    input.mi.dy = static_cast<LONG>(roundf(easedDy));
